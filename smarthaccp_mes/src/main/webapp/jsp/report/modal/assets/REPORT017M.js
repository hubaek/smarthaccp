 ...
 * 1. 작성자  	: 
 * 2. 작성일		: 
 * 3. Comment 	: 모니터링 및 검사장비 자체 검ㆍ교정 일지
 * 4. 변경이력 : 
 *  			이름		|일자          		|변경내용
 *              ------------------------------------------------------
 *				
    ...
    PAGE_ADD: function (caller, act, data) {
    	...
    	.call({
            ...
            callback: function (res) {
            	...
            	var mitem001 = res.map.recentlyTemplate.ditem001;
            	var mitem003 = res.map.recentlyTemplate.ditem003;
            	var mitem004 = res.map.recentlyTemplate.ditem004;
            	var mitem005 = res.map.recentlyTemplate.ditem005;
            	
            	fnObj.formView01.model.set("startDate",startDate);
            	fnObj.formView01.model.set("mitem001",mitem001);
            	fnObj.formView01.model.set("mitem003",mitem003);
            	fnObj.formView01.model.set("mitem004",mitem004);
            	fnObj.formView01.model.set("mitem005",mitem005);
...
fnObj.gridView02 = ppmboot.viewExtend(ppmboot.gridView, {
    initView: function () {
        ...
        this.target = ppmboot.gridBuilder({
            ...
            columns: [
                {key: "ditem001", label: "구분", 				editor: {type: "text"}, width: 100,align: "left"},
                {key: "ditem002", label: "검교정 온도계 값(A)", 	editor: {type: "text"}, width: 200,align: "left"},
                {key: "ditem003", label: "판넬 온도계 값(B)", 	editor: {type: "text"}, width: 200,align: "left"},
                {key: "ditem004", label: "오차(A-B)", 		editor: {type: "text"}, width: 200,align: "left"},
                {key: "ditem005", label: "보정 값", 			editor: {type: "text"}, width: 200,align: "left"},
            ],
            body: { 
                onClick: function () {
