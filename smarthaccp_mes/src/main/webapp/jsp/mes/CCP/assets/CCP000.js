...
var ACTIONS = ppmboot.actionExtend(fnObj, {
    ...
    ADD_FORM: function (caller, act, data) {
    	let seq = 1;
    	let arr = caller.gridView01.getData();
    	ppmboot.modal.open({  
...
fnObj.gridView01 = ppmboot.viewExtend(ppmboot.gridView, {
    initView: function () {
        ...
        this.target = ppmboot.gridBuilder({
            ...
            columns: [  
                 {key: "ccpCd", label: "주기코드", width: 100, formatter:function(){
                  	return "<font style='cursor:pointer'><u>"+nvl(this.value,'')+"</u></font>";
                 }},
                 {key: "ccpNm", label: "주기명", width: 200, align: "left"},
                 {key: "ccpDate", label: "기준일자", width: 150, align: "cneter"},
                 {key: "ccpCycle", label: "주기일", width: 100, align: "cneter"},
                 {key: "comment", label: "설명", width: 200, align: "left"},
            ],
            body: {
            	onClick: function () {
                    this.self.select(this.dindex);
                    if(this.column.key == "ccpCd") {
