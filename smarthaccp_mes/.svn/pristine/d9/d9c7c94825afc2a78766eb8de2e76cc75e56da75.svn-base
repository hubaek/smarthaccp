...
var ACTIONS = ppmboot.actionExtend(fnObj, {
    ...
    PAGE_SEARCH: function (caller, act, data) {
    	...
    	var saCustCd = param.custCd;

    	ppmboot.ajax({
            type: "GET",
            url: ["item"],
            data: $.extend({itemTypeGroup:nvl(itemTypeGroup,''), saCustCd:nvl(saCustCd,''), useYn:"Y"}, getSerializeArrayToJson("#searchView0")),
    ...
    initView: function () {
        ...
        this.target = ppmboot.gridBuilder({
            ...
            columns: [
                ...
                {key: "spec"},   
                {key: "unit"},
                {key: "saAmt", label:"단가" , formatter:"number", align: "right" },
                {key: "qcWay"},
            ],
            body: {
                onClick: function () {
