...
//        });
	...
	PRINT_POPUP: function(caller, act,data){
    	if(caller.formView01.getData() != undefined){
    		...
    		window.open("../print/REPORT004_P.jsp?reportDate="+reportDate+"&mSeq=" + mseq);
    	}else{
    		axDialog.alert({
    			theme: "primary",
    ...
    PAGE_ADD: function (caller, act, data) {
    	...
    	.call({
            ...
            data: {company:param.company, templateId:param.templateId, typeCd:'M'},
            callback: function (res) {
            	//caller.formView01.setData(res);
            	var templateId = res.map.recentlyTemplate.templateId;
            	var mseq = res.map.recentlyTemplate.mseq;
            	var reportDate = res.map.recentlyTemplate.reportDate;
            	var startDate = res.map.recentlyTemplate.startDate;
            	
            	fnObj.formView01.model.set("templateId",templateId);
            	fnObj.formView01.model.set("mseq",mseq);
            	fnObj.formView01.model.set("reportDate",reportDate);
...
fnObj.gridView02 = ppmboot.viewExtend(ppmboot.gridView, {
    initView: function () {
        ...
        this.target = ppmboot.gridBuilder({
            ...
            columns: [
            	{
                    label: "검사성적결과", columns: [
                        {label:"위치＼구분", key: "ditem001", width: 150, editor: "text"},    
                        {label:"검사항목", key: "ditem002", width: 150, editor: "text"},    
                        {label:"검사기준", key: "ditem003", width: 150, editor: "text"},
                        {label: "검사결과" , columns:[
                    		{key: "ditem004", label: "청소전",width: 70, sortable: false, align: "center", editor: {type: "text", config: {height: 17, trueValue: "Y", falseValue: "N"}}},
                            {key: "ditem005", label: "청소후",width: 70, sortable: false, align: "center", editor: {type: "text", config: {height: 17, trueValue: "N", falseValue: "Y"}}}
        	            	]
                    	},
                    ]
                },		  
                {key: "ditem006", label: "판정 결과(적합/부적합)", width: 500, editor: "text"},
            ],
            body: {
            	onClick: function () {  
