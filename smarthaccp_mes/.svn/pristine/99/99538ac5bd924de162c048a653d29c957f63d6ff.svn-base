...
var ACTIONS = ppmboot.actionExtend(fnObj, {
    ...
    PAGE_CHOICE: function (caller, act, data) {
        ...
        var printQty = g[0].printQty;
        var maxNum = g[0].useQty;
...
fnObj.pageStart = function () {
    ...
    setDefault(parseInt(Number(param.obj.maxNum) / Number(param.obj.num)));
};
...
fnObj.gridView01 = ppmboot.viewExtend(ppmboot.gridView, {
    initView: function () {
        ...
        this.target = ppmboot.gridBuilder({
            ...
            columns: [
                {key: "useQty",label: "발행가능 장 수", width: 250, align: "center"},
                {key: "printQty", label: "발행 장 수", width: 180, formatter:"number", align: "right",formatter:function(){
    ...
    setDefault:function(data){
    	...
    	fnObj.gridView01.target.setValue(0,"useQty", data.useQty);
    	fnObj.gridView01.target.select(0);
    },
    setCalcResult:function(data){
    	fnObj.gridView01.target.setValue(0,"printQty", data.printQty);
    	fnObj.gridView01.target.select(0);
    },
...
function setDefault(useQty){
	fnObj.gridView01.setDefault({useQty:useQty});
}

function setCalcResult(v){
	fnObj.gridView01.setCalcResult({printQty:v});
}
