...
fnObj.pageStart = function () {
    ...
    if(param.obj.unit == param.obj.unit){
        $("#bom_unit_btn").val(param.obj.unit);
    }else{
        $("#bom_unit_btn").val(param.obj.unit);
    ...
    setDefault(param.obj.itemNm,param.obj.stockQty + " " + param.obj.unit,param.obj.stockQty);
    setCalcUnit(param.obj.unit);
};
...
fnObj.gridView01 = ppmboot.viewExtend(ppmboot.gridView, {
    initView: function () {
        ...
        this.target = ppmboot.gridBuilder({
            ...
            columns: [
                {key: "itemCd"},
                {key: "etc1",label: "출고가능수량", width: 120, align: "center"},
                {key: "unit"},
                {key: "outQty", label: "출고수량", width: 180, formatter:"number", align: "right",formatter:function(){
    ...
    setDefault:function(data){
    	...
    	fnObj.gridView01.target.setValue(0,"itemCd", data.itemCd);
    	fnObj.gridView01.target.setValue(0,"etc1", data.etc1);
    	fnObj.gridView01.target.setValue(0,"outQty", data.stockQty);
    ...
    setCalcUnit:function(data){
    	fnObj.gridView01.target.setValue(0,"unit", data.unit);
    	fnObj.gridView01.target.select(0);
    },
...
function setDefault(itemCd,etc1,stockQty){
	fnObj.gridView01.setDefault({itemCd:itemCd,etc1:etc1,stockQty:stockQty});
}

function setCalcResult(v){
	fnObj.gridView01.setCalcResult({outQty:v});
}
function setCalcUnit(v){
	fnObj.gridView01.setCalcUnit({unit:v});
}
