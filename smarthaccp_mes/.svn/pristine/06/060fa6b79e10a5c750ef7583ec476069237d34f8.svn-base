    ...
    PAGE_ADD: function (caller, act, data) {
    	...
    	.call({
            ...
            callback: function (res) {
            	...
            	var ditem001 = res.map.recentlyTemplate.ditem001;
            	var ditem003 = res.map.recentlyTemplate.ditem002;
            	
            	fnObj.formView01.model.set("startDate",startDate);
            	fnObj.formView01.model.set("mitem001",ditem001);
            	fnObj.formView01.model.set("mitem003",ditem003);
            	/*
            	ppmboot
            	.call({
            		type:"GET",
            		url: ["dailyreport", "template"],
            		data: {company:param.company, templateId:param.templateId,startDate:startDate,typeCd:"M"},
            		callback:function (res){
            			//caller.gridView01.setData(res);
            		}
            	})
...
fnObj.gridView01 = ppmboot.viewExtend(ppmboot.gridView, {
    initView: function () {
        ...
        this.target = ppmboot.gridBuilder({
            ...
            columns: [
                {key: "ditem001", label: "공정명", width: 100,align: "left"},
                {key: "ditem002", label: "관리사항", width: 500, align: "left",multiLine:true,formatter:function formatter() {
                    return this.value.replace(/\r\n/gi,"<br/>");
            	}
                },
                {key: "ditem003", label: "점검결과", editor: {type: "textarea"},width: 300, align: "center"},
                {key: "ditem004", label: "개선조치", editor: {type: "textarea"}, width: 300, align: "center"}
            ],
            body: { 
                onClick: function () {
