...
<ppm:layout name="modal">
    <jsp:attribute name="script">
        ...
        <script type="text/javascript" src="<c:url value='assets/SA040M.js'><c:param name="v" value="${v}"></c:param></c:url>"></script>
        <script type="text/javascript" src="<c:url value='/assets/js/item-common.js'><c:param name="v" value="${v}"></c:param></c:url>"></script>
    </jsp:attribute>
    <jsp:attribute name="header">
        <h1 class="title">
            <i class="cqc-browser"></i>매출등록
        </h1>
    </jsp:attribute>
    <jsp:body>
        ...
        <div class="H5"></div>
       	<ppm:tbl clazz="ax-form-tbl" minWidth="600px">  	               		
                <input type="hidden" data-ax-path="slipType" class="form-control" value="ESTIMATE"/> 
           	    <ppm:tr>
	                <ppm:td label='거래처' width="25%">
	                    <ppm:form-modal codeName="custCd" valueName="custNm"  modalType="CUST-MODAL"/>
	                </ppm:td>    
	               <ppm:td label="매출일자" width="25%" required="true">
	                	<ppm:date-picker dataPath="saDt" requiredFlag="true" title="매출일자"/>
	               </ppm:td>    
