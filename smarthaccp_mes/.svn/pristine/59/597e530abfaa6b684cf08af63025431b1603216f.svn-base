...
fnObj.pageStart = function () {
    ...
    console.log(param);
    setDefault(param.description, param.unit);
};
...
fnObj.gridView01 = ppmboot.viewExtend(ppmboot.gridView, {
    initView: function () {
        ...
        this.target = ppmboot.gridBuilder({
            ...
            columns: [
                {key: "description",label: "항목명", width: 250, align: "center"},
                {key: "liquid", label: "액빼기량", width: 180, formatter:"number", align: "right",formatter:function(){
                	return "<font size = 6>"+nvl(this.value,0)+"</font>";
                }},
                {key: "unit",label: "단위", width: 250, align: "center"},
            ],
            body: {
                columnHeight: 60,
            },
    ...
    setDefault:function(data){
    	...
    	fnObj.gridView01.target.setValue(0,"description", data.description);
    	fnObj.gridView01.target.setValue(0,"unit", data.unit);
    	fnObj.gridView01.target.select(0);
    },
    setCalcResult:function(data){
    	fnObj.gridView01.target.setValue(0,"liquid", data.liquid);
    	fnObj.gridView01.target.select(0);
    },
...
function setDefault(description,unit){
	fnObj.gridView01.setDefault({description:description,unit:unit});
}

function setCalcResult(v){
	fnObj.gridView01.setCalcResult({liquid:v});
}
