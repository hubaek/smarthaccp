...
fnObj.formView01 = ppmboot.viewExtend(ppmboot.formView, {    
    getDefaultData: function () {		
    	var today = getNowDt();
        return $.extend({}, ppmboot.formView.defaultData, {slipDt:today,saDt:today,surtaxYn:"Y"});
    },
    initView: function () {
...
function customFormModalCallback(data){
  ...
  data.forEach(function (n) {    	
  	if(custIdx==0){
  		custObj.custCd = n.custCd;
  		custObj.custNm = n.custNm;
  	}
  ...
  fnObj.formView01.setDefaultCust(custObj);    
	axDialog.alert("적용 완료 되었습니다.");
	customModal.close();
}
